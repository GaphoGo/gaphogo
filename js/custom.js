...
function transformPrev(event) {
  ...
  /* classList.clientWidth < (liList.length * 170 + Number(activeLi)) 의미는
   * 오른쪽으로 나열될 카드들이 넘친 상태이므로, 왼쪽으로 이동이 가능함
   */
  
   if (classList.clientWidth < (liList.length * 170 + Number(activeLi))) {
...
for (let i = 0; i < slidePrevList.length; i++) {
    ...
    if (classList.clientWidth < (liList.length * 170)) {
